...
#define NFB_DYNHDR_ITEM(hdr, item, mask_width) \
	((hdr[item ## _off] >> item ## _shift) & ((1 << (mask_width)) - 1))
...
static inline void
nfb_eth_ndp_rx_df_header_fill(struct rte_mbuf *mbuf, uint16_t hdr_len, uint16_t flags)
{
	uint16_t *df_hdr_off;
	uint16_t *df_hdr_len;
	uint16_t *df_flags;
	...
	df_flags   = RTE_MBUF_DYNFIELD(mbuf, nfb_ndp_df_flags, uint16_t*);
...
static inline void
nfb_rx_fetch_fields(struct ndp_rx_queue *ndp, struct rte_mbuf *mbuf,
		uint8_t hdr_id, uint16_t hdr_len, void* header)
{
	struct ndp_rx_offload_parser *ofp;
	if (hdr_id < NDP_RXHDR_CNT) {
		ofp = &ndp->ofp[hdr_id];
		if (hdr_len >= ofp->hdr_minlen) {
			nfb_rx_fetch_timestamp(ofp, mbuf, header);
			nfb_rx_fetch_metadata(ofp, mbuf, header);
		}
	}
}
	...
	uint16_t nb_pkts)
{
	...
	for (i = 0; i < nb_pkts; ++i) {
		...
		if (likely(packet_size <= buf_size)) {
			...
			if (ndp->df_header_enable) {
				nfb_eth_ndp_rx_df_header_fill(mbuf, hdr_len, packets[i].flags);
				rte_memcpy(rte_pktmbuf_mtod(mbuf, void *),
						packets[i].header, hdr_len);
			}
			...
			nfb_rx_fetch_fields(ndp, mbuf, hdr_id, hdr_len, packets[i].header);
