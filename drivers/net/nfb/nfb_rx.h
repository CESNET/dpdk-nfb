		...
		const unsigned char *header, int header_length)
{
	...
	*nfb_timestamp_dynfield(mbuf) = timestamp;
	if (header_length > q->timestamp_vld_off &&
			header[q->timestamp_vld_off] & q->timestamp_vld_mask)
	...
	uint16_t nb_pkts)
{
	...
	for (i = 0; i < nb_pkts; ++i) {
		...
		if (likely(packet_size <= buf_size)) {
			...
			nfb_rx_fetch_timestamp(ndp, mbuf,
					packets[i].header, packets[i].header_length);
